#' Derivative of PMF with respect to $\lambda_3$
#' Author(s): Dean Hansen
#'
#' If you want to calculate f'(N_1,N_2) it should enter the
#' function as D.L3(N_2,N_1,L1,L2,L3,rhoo)
#'
#' @param x 
#' @param y 
#' @param L1 
...
#'
#' @examples
D.L3 <- function(x,y,L1,L2,L3,rhoo) {
  ...
  for (l in (j:x)) {
    jm <- l+1
    A[im,jm] <- rhoo*A[im,jm-1] + a(i)*A[im-1,jm] - v_prime(i)*pmf.M(l-1,i-1,L1,L2,L3,rhoo)[i,l] - v(i)*A[im-1,jm-1]
  }
  ...
  for (k in ((i+1):y)) {
    if (y < 2) {break}
    im <- k+1
    A[im,jm] <- rhoo*A[im-1,jm] + c(j)*A[im,jm-1] - w_prime(j)*pmf.M(j-1,k-1,L1,L2,L3,rhoo)[k,j] - w(j)*A[im-1,jm-1]
  }
  #recursion loop
  counter <- 2
  while (counter <= min(x,y)) {
    ...
    for (l in (j:x)) {
      jm <- l+1
      A[im,jm] <- rhoo*A[im,jm-1] + a(i)*A[im-1,jm] - b(i)*(A[im-2,jm] - rhoo*A[im-2,jm-1]) - v_prime(i)*pmf.M(l-1,i-1,L1,L2,L3,rhoo)[i,l] - v(i)*A[im-1,jm-1]
    }
    ...
    for (k in ((i+1):y)) {
      im <- k+1
      A[im,jm] <- rhoo*A[im-1,jm] + c(j)*A[im,jm-1] - b(j)*(A[im,jm-2] - rhoo*A[im-1,jm-2]) - w_prime(j)*pmf.M(j-1,k-1,L1,L2,L3,rhoo)[k,j] - w(j)*A[im-1,jm-1]
    }
