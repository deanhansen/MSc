#' Second derivative of PMF with respect to $\lambda_2$
#' Author(s): Dean Hansen
#'
#' If you want to calculate f''(N_1,N_2) it should enter the
#' function as DD.L2(N_2,N_1,L1,L2,L3,rhoo)
#'
#' @param x 
#' @param y 
#' @param L1 
...
#'
#' @examples
DD.L2 <- function(x,y,L1,L2,L3,rhoo) {
  ...
  c_prime <- function(j) {(1-rhoo)/j}
  b_prime <- function(i) {0}
  v_prime <- function(i) {0}
  w_prime <- function(j) {(rhoo*(1 - rhoo))/j}
  ...
  j<-counter+1; jm<-j+1
  if (x != 0) {
    A[im,jm] <- 2*c_prime(j)*D.L2(j-1,i,L1,L2,L3,rhoo)[i+1,j] + c(j)*A[im,jm-1]
  }
  # f(0,j)
  # j=2,3,...;
  for (l in ((j+1):x)) {
    if (x <= 1) {break}
    jm <- l+1
    A[im,jm] <- 2*c_prime(l)*D.L2(l-1,i,L1,L2,L3,rhoo)[i+1,l] + c(l)*A[im,jm-1] - b(l)*A[im,jm-2]
  }
  ...
  for (k in ((i+1):y)) {
    if (y < 2) {break}
    im <- k+1
    A[im,jm] <- rhoo*A[im-1,jm] + 2*c_prime(j)*D.L2(j-1,k,L1,L2,L3,rhoo)[k+1,j] + c(j)*A[im,jm-1] - 2*w_prime(j)*D.L2(j-1,k-1,L1,L2,L3,rhoo)[k,j] - w(j)*A[im-1,jm-1]
  }
  #recursion loop
  counter <- 2
  while (counter <= min(x,y)) {
    ...
    for (k in ((i+1):y)) {
      im <- k+1
      A[im,jm] <- rhoo*A[im-1,jm] + 2*c_prime(j)*D.L2(j-1,k,L1,L2,L3,rhoo)[k+1,j] + c(j)*A[im,jm-1] - b(j)*(A[im,jm-2] - rhoo*A[im-1,jm-2]) - 2*w_prime(j)*D.L2(j-1,k-1,L1,L2,L3,rhoo)[k,j] - w(j)*A[im-1,jm-1]
    }
