#' Derivative of PMF with respect to $\lambda_2$
#' Author(s): Dean Hansen
#'
#' If you want to calculate f'(N_1,N_2) it should enter the
#' function as D.L2(N_2,N_1,L1,L2,L3,rhoo)
#'
#' @param x 
#' @param y 
#' @param L1 
...
#'
#' @examples
D.L2 <- function(x,y,L1,L2,L3,rhoo) {
  ...
  for (l in ((j+1):x)) {
    if (x <= 1) {break}
    jm <- l+1
    A[im,jm] <- c_prime(l)*pmf.M(l-1,i,L1,L2,L3,rhoo)[i+1,l] + c(l)*A[im,jm-1] - b(l)*A[im,jm-2]
  }
  ...
  for (k in ((i+1):y)) {
    if (y < 2) {break}
    im <- k+1
    A[im,jm] <- rhoo*A[im-1,jm] + c_prime(j)*pmf.M(j-1,k,L1,L2,L3,rhoo)[k+1,j] + c(j)*A[im,jm-1] - w_prime(j)*pmf.M(j-1,k-1,L1,L2,L3,rhoo)[k,j] - w(j)*A[im-1,jm-1]
  }
  #recursion loop
  counter <- 2
  while (counter <= min(x,y)) {
    ...
    for (k in ((i+1):y)) {
      im <- k+1
      A[im,jm] <- rhoo*A[im-1,jm] + c_prime(j)*pmf.M(j-1,k,L1,L2,L3,rhoo)[k+1,j] + c(j)*A[im,jm-1] - b(j)*(A[im,jm-2] - rhoo*A[im-1,jm-2]) - w_prime(j)*pmf.M(j-1,k-1,L1,L2,L3,rhoo)[k,j] - w(j)*A[im-1,jm-1]
    }
