#' Derivative of PMF with respect to $\lambda_1$
#' Author(s): Dean Hansen
#'
#' If you want to calculate f'(N_1,N_2) it should enter the
#' function as D.L1(N_2,N_1,L1,L2,L3,rhoo)
#'
#' @param x 
#' @param y 
#' @param L1 
...
#'
#' @examples
D.L1 <- function(x,y,L1,L2,L3,rhoo) {
  ...
  for (k in ((i+1):y)) {
    if (y<=1) {break}
    im <- k+1
    A[im,jm] <- a_prime(k)*pmf.M(j,k-1,L1,L2,L3,rhoo)[k,j+1] + a(k)*A[im-1,jm] - b(k)*A[im-2,jm]
  }
  ...
  for (l in (j:x)) {
    jm <- l+1
    A[im,jm] <- rhoo*A[im,jm-1] + a_prime(i)*pmf.M(l,i-1,L1,L2,L3,rhoo)[i,l+1] + a(i)*A[im-1,jm] - v_prime(i)*pmf.M(l-1,i-1,L1,L2,L3,rhoo)[i,l] - v(i)*A[im-1,jm-1]
  }
  ...
  while (counter <= min(x,y)) {
    ...
    for (l in (j:x)) {
      jm <- l+1
      A[im,jm] <- rhoo*A[im,jm-1] + a_prime(i)*pmf.M(l,i-1,L1,L2,L3,rhoo)[i,l+1] + a(i)*A[im-1,jm] - b(i)*(A[im-2,jm] - rhoo*A[im-2,jm-1]) - v_prime(i)*pmf.M(l-1,i-1,L1,L2,L3,rhoo)[i,l] - v(i)*A[im-1,jm-1]
    }
