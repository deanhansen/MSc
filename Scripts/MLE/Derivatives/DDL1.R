#' Second derivative of PMF with respect to $\lambda_1$
#' Author(s): Dean Hansen
#'
#' If you want to calculate f''(N_1,N_2) it should enter the
#' function as DD.L1(N_2,N_1,L1,L2,L3,rhoo)
#'
#' @param x 
#' @param y 
#' @param L1 
...
#'
#' @examples
DD.L1 <- function(x,y,L1,L2,L3,rhoo) {
  ...
  a_prime <- function(i) {(1 - rhoo)/i}
  c_prime <- function(j) {0}
  b_prime <- function(i) {0}
  v_prime <- function(i) {(rhoo*(1-rhoo))/i}
  w_prime <- function(j) {0}
  ...
  j<-counter; jm<-j+1
  if (y!=0) {
    A[im,jm] <- 2*a_prime(i)*D.L1(j,i-1,L1,L2,L3,rhoo)[i,j+1] + a(i)*A[im-1,jm]
  }
  # f(i,0)
  # i=2,3,...;
  for (k in ((i+1):y)) {
    if (y<=1) {break}
    im <- k+1
    A[im,jm] <- 2*a_prime(k)*D.L1(j,k-1,L1,L2,L3,rhoo)[k,j+1] + a(k)*A[im-1,jm] - b(k)*A[im-2,jm]
  }
  ...
  for (l in (j:x)) {
    jm <- l+1
    A[im,jm] <- rhoo*A[im,jm-1] + 2*a_prime(i)*D.L1(l,i-1,L1,L2,L3,rhoo)[i,l+1] + a(i)*A[im-1,jm] - 2*v_prime(i)*D.L1(l-1,i-1,L1,L2,L3,rhoo)[i,l] - v(i)*A[im-1,jm-1]
  }
  ...
  while (counter <= min(x,y)) {
    ...
    for (l in (j:x)) {
      jm <- l+1
      A[im,jm] <- rhoo*A[im,jm-1] + 2*a_prime(i)*D.L1(l,i-1,L1,L2,L3,rhoo)[i,l+1] + a(i)*A[im-1,jm] - b(i)*(A[im-2,jm] - rhoo*A[im-2,jm-1]) - 2*v_prime(i)*D.L1(l-1,i-1,L1,L2,L3,rhoo)[i,l] - v(i)*A[im-1,jm-1]
    }
